...
import {
  ...
  createMigrationGroup, createGroup, getApplications, getImportedApps, lookupIPMapping,
  getIPMappings, importIPMappings, compileEgressIngress,
  getFilteredNhSzDc, validateAndCreateGroups, getAutoGeneratedGroups,
  getAppMigrationStatus, getAppDCMappings, submitGroupPolicyChanges,
} from '@/lib/api';
...
export function MigrationStudioPage() {
  ...
  const [autoGroups, setAutoGroups] = useState<Record<string, unknown>[]>([]);
  ...
  // Partial migration scenario state
  const [appMigrationInfo, setAppMigrationInfo] = useState<AppMigrationSummary | null>(null);
  ...
  // App Groups modal
  const [showAppGroupsModal, setShowAppGroupsModal] = useState(false);

  // Settings App Management mappings (Issue 6 - dynamic DC/component data for migration)
  const [appDcMappings, setAppDcMappings] = useState<Record<string, string>[]>([]);
  ...
  // Load partial migration info and Settings App Management mappings when app changes
  useEffect(() => {
    if (!selectedApp) { setAppMigrationInfo(null); setAppDcMappings([]); return; }
    getAppMigrationStatus(selectedApp)
  ...
  const loadIPMappings = useCallback(async (appFilter?: string) => {
    ...
    try {
      const [data, groups] = await Promise.all([
        getIPMappings(undefined, appFilter || undefined),
        getAutoGeneratedGroups(appFilter || undefined),
      ]);
      setAllIPMappings(data);
      setAutoGroups(groups);
    } catch { setAllIPMappings([]); setAutoGroups([]); }
  ...
  // Derive available DCs for this app from Settings App Management mappings (Issue 6)
  const migrationAppDCs = (() => {
    const dcSet = new Set<string>();
    for (const m of appDcMappings) {
      if (m.dc) dcSet.add(m.dc);
    }
    return Array.from(dcSet);
  })();

  // Derive available components for this app from Settings App Management mappings
  const migrationAppComponents = (() => {
    const compSet = new Set<string>();
    for (const m of appDcMappings) {
      if (m.component) compSet.add(m.component);
    }
    return Array.from(compSet);
  })();
  void migrationAppComponents; // Used in group creation UI below

  // Create new group during migration — also registers in App Groups (Manage Groups)
  // Issue 6: Uses Settings App Management mappings for DC-based group creation
  const handleCreateMigrationGroup = async () => {
    ...
    try {
      ...
      // Also register in App Groups (Manage Groups) for strict enforcement
      try {
        const appDistId = migrateRule.app_distributed_id || String(migrateRule.app_id);
        // Parse naming standard: grp-{APP}-{NH}-{SZ}-{Component}
        const parts = newGroupName.split('-');
        const component = parts.length >= 5 ? parts[4] : '';
        await createGroup({
          name: newGroupName,
          app_id: appDistId,
          nh: selectedNh || (parts.length >= 3 ? parts[2] : ''),
          sz: selectedSz || (parts.length >= 4 ? parts[3] : ''),
          subtype: component,
          description: `Auto-created during migration for ${appDistId}`,
          members: validMembers.map(m => ({ type: m.type, value: m.value, description: '' })),
        });
      } catch { /* Group may already exist in App Groups — that's fine */ }
      ...
      if (appMigrationInfo && migrationAppDCs.length > 0) {
        ...
        const targetDCs = isPartial
          ? migrationAppDCs.filter(dc => {
              ...
              const migDCs = (appMigrationInfo as unknown as Record<string, string[]>).migrating_dcs || [];
              return migDCs.includes(dc);
            })
          : migrationAppDCs; // Complete migration: all DCs

        // Create groups for each target DC if naming standard allows
        for (const dc of targetDCs) {
          const dcGroupName = newGroupName.replace(/^grp-/, `grp-${dc}-`);
          if (dcGroupName !== newGroupName) {
            try {
              await createGroup({
                name: dcGroupName,
                app_id: migrateRule.app_distributed_id || String(migrateRule.app_id),
                nh: selectedNh, sz: selectedSz, subtype: '',
                description: `Auto-created for DC ${dc} during ${isPartial ? 'partial' : 'complete'} migration`,
                members: validMembers.map(m => ({ type: m.type, value: m.value, description: '' })),
              });
            } catch { /* DC-specific group may already exist */ }
          }
        }
      }

      // Submit policy change for review & approval (group creation triggers review)
      try {
        const memberValues = validMembers.map(m => m.value);
        await submitGroupPolicyChanges(
          newGroupName,
          'group_created',
          `Group ${newGroupName} created during migration for ${migrateRule.app_distributed_id || migrateRule.app_id}. Members: ${memberValues.join(', ')}`,
          { added: { [`group:${newGroupName}`]: memberValues }, removed: {}, changed: {} }
        );
    ...
    <div className="p-6 max-w-[1800px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShowAppGroupsModal(true)} className="px-4 py-2 text-sm font-medium text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-md hover:bg-indigo-100">
            App Groups
          </button>
      ...
      {appMigrationInfo && appMigrationInfo.migration_scenario !== 'Full NGDC' && (
        <div className={`mb-4 border rounded-lg p-3 ${appMigrationInfo.migration_scenario === 'Full Legacy' ? 'bg-amber-50 border-amber-300' : 'bg-purple-50 border-purple-300'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className={`px-2 py-0.5 rounded text-xs font-bold ${appMigrationInfo.migration_scenario === 'Full Legacy' ? 'bg-amber-200 text-amber-800' : 'bg-purple-200 text-purple-800'}`}>
                {appMigrationInfo.migration_scenario === 'Full Legacy' ? 'LEGACY ONLY' : 'PARTIAL MIGRATION'}
              </span>
              <span className="text-sm font-medium text-gray-700">
                {appMigrationInfo.app_id} ({appMigrationInfo.app_distributed_id})
              </span>
              <span className="text-xs text-gray-500">
                {appMigrationInfo.ngdc_count} NGDC / {appMigrationInfo.legacy_count} Legacy components
              </span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-24 bg-gray-200 rounded-full h-2">
                <div className="bg-green-500 h-2 rounded-full" style={{ width: `${appMigrationInfo.pct_migrated}%` }} />
              </div>
              <span className="text-xs font-medium text-gray-600 ml-1">{appMigrationInfo.pct_migrated}%</span>
            </div>
          </div>
          {appMigrationInfo.migration_scenario === 'Partial' && (
            <div className="mt-2 flex flex-wrap gap-1">
              {appMigrationInfo.components.map(c => (
                <span key={c.component} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium ${c.dc_location === 'NGDC' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-orange-100 text-orange-700 border border-orange-200'}`}>
                  {c.component}: {c.dc_location === 'NGDC' ? c.dc : c.legacy_dc}
                  <span className={`w-1.5 h-1.5 rounded-full ${c.dc_location === 'NGDC' ? 'bg-green-500' : 'bg-orange-500'}`} />
                </span>
              ))}
            </div>
          )}
      ...
      {viewMode === 'group-mappings' ? (
        <div className="space-y-4">
          {/* Header with App Filter + Import + Auto-Create */}
          <div className="bg-white border rounded-lg shadow-sm">
            <div className="px-4 py-3 border-b">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <h2 className="text-sm font-semibold text-gray-800">Group Mappings (Auto-Generated from IP Mappings)</h2>
                  <p className="text-xs text-gray-500 mt-0.5">Import IP mappings (App, Component, Legacy IP, NGDC IP). NGDC IPs are validated against CIDR ranges. Groups are auto-created per naming standards.</p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <label className="px-3 py-1.5 text-xs font-medium text-emerald-700 bg-emerald-50 border border-emerald-200 rounded hover:bg-emerald-100 cursor-pointer">
                    ...
                    <input type="file" accept=".csv,.json" className="hidden" onChange={async (e) => {
                      ...
                      try {
                        ...
                        const msgs: string[] = [`Imported ${result.added} mappings`];
                        if (result.validated > 0) msgs.push(`${result.validated} CIDR-validated`);
                        if (result.invalid > 0) msgs.push(`${result.invalid} failed CIDR validation`);
                        showNotification(msgs.join(', '), result.invalid > 0 ? 'warning' : 'success');
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        const result = await validateAndCreateGroups(groupMappingsFilter || undefined);
                        showNotification(`Auto-created ${result.groups_created} groups, updated ${result.groups_updated} (${result.revalidated} IPs validated)`, 'success');
                        loadIPMappings(groupMappingsFilter);
                      } catch { showNotification('Failed to auto-create groups', 'error'); }
                      setCreatingGroups(false);
                    }}
                    disabled={creatingGroups || allIPMappings.length === 0}
                    className="px-3 py-1.5 text-xs font-medium text-white bg-indigo-600 rounded hover:bg-indigo-700 disabled:opacity-50"
                  >
                    {creatingGroups ? 'Creating...' : 'Auto-Create Groups'}
                  </button>
                </div>
              </div>
              {/* App Filter */}
              <div className="flex items-center gap-2">
                <label className="text-xs font-medium text-gray-600">Filter by App:</label>
                <select
                  value={groupMappingsFilter}
                  onChange={e => { setGroupMappingsFilter(e.target.value); }}
                  className="px-2 py-1 text-xs border border-gray-300 rounded-md min-w-[200px]"
                >
                ...
                <span className="text-[10px] text-gray-400 ml-1">
                  CSV format: app_id, component, legacy_ip, ngdc_ip
                </span>
              </div>
            </div>
          </div>

          {/* Auto-Generated Groups (Read-Only) */}
          {autoGroups.length > 0 && (
            <div className="bg-white border rounded-lg shadow-sm">
              <div className="px-4 py-3 border-b">
                <h3 className="text-sm font-semibold text-indigo-800">Auto-Generated Groups ({autoGroups.length})</h3>
                <p className="text-xs text-indigo-500 mt-0.5">Groups auto-created from validated IP mappings. Naming: grp-APP-NH-SZ-Component</p>
              </div>
              <div className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {autoGroups.map((g, i) => {
                    const members = (g.members || []) as { type: string; value: string }[];
                    return (
                      <div key={i} className="border border-indigo-200 rounded-lg p-3 bg-indigo-50/50">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-xs font-mono font-bold text-indigo-800">{String(g.name || '')}</span>
                          <span className="text-[9px] px-1.5 py-0.5 bg-indigo-100 text-indigo-600 rounded font-medium">AUTO</span>
                        </div>
                        <div className="flex flex-wrap gap-1 mb-1.5">
                          {g.nh ? <span className="text-[9px] px-1 py-0.5 bg-blue-100 text-blue-700 rounded">NH: {String(g.nh)}</span> : null}
                          {g.sz ? <span className="text-[9px] px-1 py-0.5 bg-purple-100 text-purple-700 rounded">SZ: {String(g.sz)}</span> : null}
                          {g.component ? <span className="text-[9px] px-1 py-0.5 bg-amber-100 text-amber-700 rounded">{String(g.component)}</span> : null}
                        </div>
                        <div className="space-y-0.5">
                          {members.slice(0, 5).map((mem, j) => (
                            <div key={j} className="text-[10px] font-mono text-gray-600 flex items-center gap-1">
                              <span className={`w-1.5 h-1.5 rounded-full ${mem.type === 'subnet' ? 'bg-purple-400' : mem.type === 'range' ? 'bg-orange-400' : 'bg-green-400'}`} />
                              {mem.value}
                            </div>
                          ))}
                          {members.length > 5 && <div className="text-[9px] text-gray-400">+{members.length - 5} more members</div>}
                        </div>
                        {/* Subcomponent naming for nested groups (src/dst) */}
                        <div className="mt-2 pt-1.5 border-t border-indigo-200 flex gap-1">
                          <span className="text-[9px] px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded border border-blue-200 font-mono">{String(g.name)}-src</span>
                          <span className="text-[9px] px-1.5 py-0.5 bg-purple-50 text-purple-600 rounded border border-purple-200 font-mono">{String(g.name)}-dst</span>
                        </div>
                      </div>
                    );
                  })}
          ...
          {/* IP Mappings Table (Read-Only with CIDR validation) */}
          <div className="bg-white border rounded-lg shadow-sm">
            <div className="px-4 py-3 border-b">
              <h3 className="text-sm font-semibold text-gray-800">IP Mappings ({allIPMappings.length})</h3>
              <p className="text-xs text-gray-500 mt-0.5">NGDC IPs validated against AppDCMapping CIDR ranges. Green = valid, Red = no matching CIDR.</p>
            </div>
            <div className="p-4">
              {loadingMappings ? (
                  ...
                  <table className="min-w-full text-xs">
                    <thead>
                      <tr className="bg-gray-50 border-b">
                        <th className="px-3 py-2 text-left font-semibold text-gray-600">CIDR</th>
                        ...
                        <th className="px-3 py-2 text-left font-semibold text-gray-600">Auto Group</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {allIPMappings.map((m, i) => (
                        <tr key={i} className={`hover:bg-gray-50 ${m.cidr_valid === false ? 'bg-red-50/50' : ''}`}>
                          <td className="px-3 py-1.5 text-center">
                            {m.cidr_valid === true ? (
                              <span className="inline-block w-2.5 h-2.5 rounded-full bg-green-500" title={`Valid: ${String(m.cidr || '')}`} />
                            ) : m.cidr_valid === false ? (
                              <span className="inline-block w-2.5 h-2.5 rounded-full bg-red-500" title="No matching CIDR range" />
                            ) : (
                              <span className="inline-block w-2.5 h-2.5 rounded-full bg-gray-300" title="Not validated" />
                            )}
                          </td>
                          <td className="px-3 py-1.5 font-medium">{String(m.app_id || '')}</td>
                          <td className="px-3 py-1.5">
                            <span className="px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded text-[10px] font-medium">{String(m.component || '-')}</span>
                          </td>
                          ...
                          <td className="px-3 py-1.5 text-gray-600">{String(m.ngdc_nh || '')}</td>
                          <td className="px-3 py-1.5 text-gray-600">{String(m.ngdc_sz || '')}</td>
                          <td className="px-3 py-1.5">
                            {m.ngdc_group ? <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 rounded text-[10px] font-medium">{String(m.ngdc_group)}</span> : <span className="text-gray-300">-</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="text-center py-12 text-gray-400">
                  <p className="text-sm">No IP mappings found{groupMappingsFilter ? ` for app ${groupMappingsFilter}` : ''}.</p>
                  <p className="text-xs mt-1">Import CSV with columns: app_id, component, legacy_ip, ngdc_ip</p>
                </div>
              )}
            </div>
            <div className="px-4 py-2 border-t bg-gray-50 text-xs text-gray-500 flex items-center justify-between">
              <span>
                {allIPMappings.length} mapping{allIPMappings.length !== 1 ? 's' : ''} |
                {' '}{allIPMappings.filter(m => m.cidr_valid === true).length} validated |
                {' '}{autoGroups.length} auto-groups
              </span>
              <span>Naming: grp-APP-NH-SZ-Component[-src|-dst]</span>
            </div>
          </div>
        </div>
      ) : !selectedApp ? (
                        ...
                        <div className="space-y-3">
                          ...
                          {srcGroups.map((cg, i) => (
                            <div key={`src-${i}`} className="border border-green-200 rounded-lg p-3 bg-white">
                              ...
                              {/* Subcomponent naming for nested groups */}
                              <div className="mt-1.5 pt-1.5 border-t border-green-200">
                                <span className="text-[9px] text-gray-500">Subcomponent groups:</span>
                                <div className="flex gap-1 mt-0.5">
                                  <span className="text-[9px] px-1.5 py-0.5 bg-blue-50 text-blue-700 rounded border border-blue-200 font-mono font-bold">{cg.ngdc_group}-src</span>
                                  <span className="text-[9px] px-1.5 py-0.5 bg-gray-50 text-gray-400 rounded border border-gray-200 font-mono">{cg.ngdc_group}-dst</span>
                                </div>
                              </div>
                            </div>
                          ))}
                          ...
                          {dstGroups.map((cg, i) => (
                            <div key={`dst-${i}`} className="border border-purple-200 rounded-lg p-3 bg-white">
                              ...
                              <div className="mt-1.5 pt-1.5 border-t border-purple-200">
                                <span className="text-[9px] text-gray-500">Subcomponent groups:</span>
                                <div className="flex gap-1 mt-0.5">
                                  <span className="text-[9px] px-1.5 py-0.5 bg-gray-50 text-gray-400 rounded border border-gray-200 font-mono">{cg.ngdc_group}-src</span>
                                  <span className="text-[9px] px-1.5 py-0.5 bg-purple-50 text-purple-700 rounded border border-purple-200 font-mono font-bold">{cg.ngdc_group}-dst</span>
                                </div>
                              </div>
                            </div>
                          ))}
