...
import { Shield, Trash2, CheckCircle, AlertTriangle, Edit, Filter, ChevronDown, ChevronUp, History, Send, Save, Eye } from 'lucide-react';
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      {!collapsed && (
        <>
          {/* App ID Filter */}
          <div className="flex items-center gap-3 border-t border-b border-slate-100 px-4 py-2">
            ...
            <table className="w-full">
              ...
              <tbody>
                ...
                {rules.map((rule) => {
                  ...
                  const isConfirmingDelete = confirmDeleteId === rule.rule_id;
                  return (
                    <tr key={rule.id || rule.rule_id} className="group border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center justify-end gap-1">
                          {/* View / Expand details */}
                          <button
                            onClick={() => setExpandedRowId(isExpanded ? null : rule.rule_id)}
                          ...
                          <button
                            ...
                            className="rounded p-1.5 text-slate-400 hover:text-purple-600 hover:bg-purple-50 transition-colors"
                            title="View history"
                          >
                          ...
                          {isConfirmingDelete ? (
                            <div className="flex items-center gap-1">
                              <button
                                onClick={() => { onDelete(rule.rule_id); setConfirmDeleteId(null); }}
                              ...
                              <button
                                ...
                                className="rounded bg-slate-200 px-2 py-1 text-xs font-medium text-slate-600 hover:bg-slate-300 transition-colors"
                              >
                                Cancel
                              </button>
                            </div>
                        ...
                        {isExpanded && (
                          <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-left">
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              ...
                              {rule.certified_at && (
                                <div className="col-span-2"><span className="font-medium text-slate-500">Certified:</span> <span className="text-slate-700">{new Date(rule.certified_at).toLocaleDateString()} by {rule.certified_by}</span></div>
                              )}
                            </div>
                            <div className="mt-2 flex items-center gap-2 border-t border-slate-200 pt-2">
                              {rule.status === 'Draft' && (
                                <>
                                  <button
                                    ...
                                    className="flex items-center gap-1 rounded bg-slate-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-slate-700 transition-colors"
                                  >
                                    <Save className="h-3 w-3" /> Save Draft
                                  </button>
                                  <button
                                    onClick={() => onSubmit(rule.rule_id)}
                                    className="flex items-center gap-1 rounded bg-blue-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-blue-700 transition-colors"
                                  >
                              ...
                              {rule.status === 'Pending Review' && onRequestReview && (
                                <button
                                  ...
                                  className="flex items-center gap-1 rounded bg-amber-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-amber-700 transition-colors"
                                >
